...
To ensure consistency in how the services are written, and to reduce the amount of boilerplate code, all the services use a variant of the `ResponseUtility.serve()` method. This allows the service to focus solely on the task of generating the service data, and not worry about creating the ResponseEntity or generating an error response. In case of an error, the service need only throw a ResponseException, which includes an HttpStatus value. By convention most of these exceptions are throws by methods beginning with the word "confirm." For example, if a service requests an item with a specific ID, and the item may return null, the service should call `PojoUtility.confirmNotNull(entity, id);` If the value is `null`, the `confirmNotNull()` method will throw a ResponseException with a NOT_FOUND status. 

So a service method that needs to return an instance of `MenuItemDto` would look something like this:

```java
1  @RequestMapping(value = "/menuItem/{id}", produces = {"application/json"}, method = RequestMethod.GET)
2  public ResponseEntity<MenuItemDto> getMenuItem(@PathVariable("id") final Integer id) {
3    return serve(HttpStatus.OK, () -> {
4      MenuItem menuItem = menuItemRepository.findOne(id);
5      confirmNotNull(menuItem, id); // throws new ResponseException(HttpStatus.NOT_FOUND, id.toString()) if menuItem is null 
...
On line 5, we test for null, using the `confirmNotNull()` method.

On line 3, The `serve()` method takes care four boilerplate details:
...
1. It generates the proper error response, with an error status code, if the `confirmNotNull()` method throws a `ResponseException`.
...
### Sample `confirmXxx()` methods. (All of these may throw a `ResponseException`.)

* `confirmAndDecodeInteger(final String id) throws ResponseException` Parses the String into an Integer
* `confirmNotNull(T entity, Object id) throws ResponseException` Confirms the returned entity with id is not null
* `confirmNeverNull(T object) throws ResponseException` Used for values that are not entities.
* `confirmNull(Object object) throws ResponseException` Used to ensure the object is null. Useful to ensure a resource doesn't already exist.
* `confirmEqual(T expected, T actual) throws ResponseException` Confirms the values are equal.
